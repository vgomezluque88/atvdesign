...
class Cookie_Law_Info_Cli_Themes {
	...
	*/
	public $btn_shortcodes = array(
		...
		'button_8' => '[cookie_save_preferences]', // Close
	);
	...
	*/
	...
	public static function cli_themeblock( $data ) {
		...
		$banner_version  = isset( $plugin_options['banner_version'] ) ? $plugin_options['banner_version'] : '2.0' ;
		...
		for ( $e = 1; $e <= 7; $e++ ) {
			...
			$btn_html          = '<a class="cli_theme_' . $btn_name . ' cli-theme-button" data-cli-id="cli_theme_' . $btn_name . '" data-cli-bartype="' . $config['bar']['type'] . '" style="' . $config[ $btn_name ]['style'] . '" title="' . $title_arr[ $e - 1 ] . '">' . $config[ $btn_name ]['text'] . '</a>';
			$short_code_html[] = $btnconf['status'] == 1 ? $btn_html : '';
			$btn_html_arr[]    = $btn_html;
		}
		$bar_hd_html = '<h5 style="' . $config['heading']['style'] . '" data-cli-id="cli_theme_hd" id="wt-cli-cookie-banner-title" class="cli_theme_hd" data-cli-bartype="' . $config['bar']['type'] . '" title="Cookie Bar heading">' . $config['heading']['text'] . '</h5>';
		...
		?>
		<div style="<?php echo $config['bar']['style']; ?>" title="Cookie Bar" data-cli-id="cli_theme_bar" class="cli_theme_bar wt-cli-cookie-bar-container wt-cli-cookie-bar" data-cli-type="<?php echo $config['bar']['type']; ?>" data-cli-bartype="<?php echo $config['bar']['type']; ?>" data-banner-version="<?php echo $banner_version; ?>">
	...
	public static function settings_tabhead( $arr ) {
		$arr['cookie-law-info-themes'] = __( 'Themes', 'webtoffee-gdpr-cookie-consent' );
		return $arr;
	}
	...
	public function ajax_cli_theme() {
		...
		$settings_params = array(
			...
			// 'button_8' => array(
			// 	'background-color' => 'button_8_button_colour',
			// 	'color'            => 'button_8_link_colour',
			// 	'border-color'            => 'button_8_border_colour',
			// ),
		);
		...
		for ( $e = 1; $e <= 7; $e++ ) {
			...
			$txt_db 				  = $btn_name . '_text';
			$the_options[ $txt_db ] = isset( $_POST[ $txt_name ] ) ? Cookie_Law_Info::sanitise_settings( $txt_db, $_POST[ $txt_name ] ) : '';
	...
	private function get_current_theme_vars( $the_options, $default_theme ) {
		...
		$settings_params = array(
			...
			// 'button_8' => array(
			// 	'button_8_button_colour' => 'background-color',
			// 	'button_8_link_colour'   => 'color',
			// 	'button_8_border_colour'   => 'border-color',
			// ),
		);
